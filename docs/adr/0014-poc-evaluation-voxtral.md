...
## Pivot stack — 2026-05-24

La voie initialement envisagée pour faire tourner Voxtral Mini 3B était `transformers ≥ 5.2` + PyTorch + AMD ROCm Windows sur la RX 7900 XT du poste. Trois jours d'investigation ont montré que cette voie est un cul-de-sac en mai 2026 — non par limite de transformers ou de Voxtral, mais par l'état du wheel `torch 2.9.1+rocm7.2.1` officiel AMD pour Windows.

**Constat technique.** Le wheel AMD officiel est compilé avec `USE_DISTRIBUTED=0` (limitation architecturale de la preview ROCm Windows — voir [ROCm/ROCm#5906](https://github.com/ROCm/ROCm/issues/5906)). À l'import de `torch`, le sous-module `torch.distributed.tensor` est chargé immédiatement et chaîne sur `torch.distributed.distributed_c10d` qui plante avec `ModuleNotFoundError: No module named 'torch._C._distributed_c10d'`. transformers ≥ 4.55 importe systématiquement `torch.distributed.tensor` au chargement de `AutoProcessor`, donc l'erreur survient avant même que Voxtral soit touché. Toutes les versions transformers compatibles Voxtral (≥ 4.54) sont affectées. La voie Python-stub permissif explorée a permis de remonter de 5 étages dans le call stack mais finit par buter sur un opérateur PyTorch C++ manquant (`_c10d_functional::all_reduce does not exist`) qui n'est pas patchable Python-level. La voie torch-DirectML est en mode maintenance depuis 2024 chez Microsoft et garderait les mêmes risques `torch.distributed`. La voie torch CPU fonctionne mais retire l'intérêt même du POC (RTF non comparable à Whisper Vulkan).

**Décision de pivot.** Bascule vers `llama.cpp` + backend Vulkan + GGUF Voxtral via `llama-mtmd-cli` (libmtmd, le pipeline multimodal de llama.cpp stabilisé depuis avril 2025). Cohérent avec l'infrastructure existante Deckle — `whisper.cpp` + Vulkan tourne déjà sur la même machine, le compilateur GCC 15.2.0 MinGW + CMake 4.3.1 + Vulkan SDK 1.4.341.1 sont en place. GPU AMD natif via Vulkan, pas ROCm. Si Voxtral gagne le POC, la phase d'intégration Deckle utilisera le même pattern P/Invoke que Whisper (`libllama.dll` ≈ `libwhisper.dll`), ce qui aligne la voie d'intégration native sur l'existant.

**Coût du pivot.** Le wrapper Python `voxtral_engine.py` initial basé sur `transformers.VoxtralForConditionalGeneration` (~200 lignes) est dévalorisé et remplacé par un wrapper subprocess vers `llama-mtmd-cli`, analogue à `voxtral_baseline_whisper.py` (~50 lignes). Le reste du harnais — configs TOML des 5 prompts, prompt judge Ollama, métriques objectives, structure du bench, snapshot des configs par run, structure de l'ADR — survit intact. Le venv Python sert toujours pour le judge Ollama et les helpers audio (librosa, soundfile, jiwer) ; il est minimisé en désinstallant torch+rocm+transformers+mistral-common qui deviennent inutiles. Le setup script `benchmark/setup-voxtral-env.ps1` est réécrit pour cloner+builder llama.cpp + télécharger les GGUF, plus créer le venv minimal.

**Réserve sur la validité de Phase 1.** La voie `llama.cpp` + `libmtmd` pour le multimodal audio est marquée comme « expérimentale, qualité variable » dans la documentation upstream. La sortie texte produite par Voxtral via cette voie peut donc être de qualité inférieure à la sortie via `transformers` natif. Phase 1 mesure donc Voxtral *as implemented via Vulkan/GGUF/libmtmd*, pas Voxtral dans l'absolu. Conséquence pratique pour l'interprétation : si Voxtral gagne malgré ce handicap probable, c'est un fort signal positif. Si Voxtral perd, l'arbitrage reste ambigu — qualité intrinsèque du modèle vs qualité de l'implémentation backend. Le rapport humain final devra trancher cette ambiguïté en lisant les sorties, pas seulement en lisant les scores agrégés.

**Conditions de levée du pivot.** Si AMD ship un torch ROCm Windows complet (avec `c10d` compilé) — suivi via [ROCm/ROCm#5689](https://github.com/ROCm/ROCm/issues/5689) — la voie transformers redeviendrait viable. Si dans ce cas Voxtral perdait via `llama.cpp` mais qu'on voudrait re-mesurer via transformers natif, un sous-jalon Phase 2bis pourrait être ouvert. Tant que ROCm Windows ne ship pas `c10d`, le POC reste sur `llama.cpp` + Vulkan.
