...
Re-éval triggers : Windows App SDK ajoute un support natif (backdrop HDR sur `Window`, brush Composition extended-range) ; OU un autre composant Deckle nécessite déjà un swap chain custom (viewport HDR debug Ambient, calibration tool) et la mutualisation devient rentable. Voir [reference--color-science-pipeline--0.1.md](reference--color-science-pipeline--0.1.md) axe 3 pour le détail.
