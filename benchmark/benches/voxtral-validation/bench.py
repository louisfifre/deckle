...
Préalable :
  - Avoir construit le corpus ``voxtral-val-30`` via
    ``benchmark/build_corpus_voxtral_val_30.py``.
  - Avoir pré-généré les références Gemini via
    ``benchmark/pregenerate_groundtruth_gemini.py`` (les samples sans
    ``reference_text_gemini`` sont skippés avec un warning).
...
def main() -> int:
    ...
    if not samples:
        print(f"FATAL : tous les samples sont sans reference_text_gemini.\n"
              f"  Lancer d'abord : python benchmark/pregenerate_groundtruth_gemini.py "
