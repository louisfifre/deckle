"""Construit le corpus ``voxtral-val-30`` à partir de la télémétrie Deckle.

Un corpus de validation stratifié par durée — 30 samples répartis sur
cinq buckets (very-short, short, medium, long, very-long-edge) — pour
mesurer Voxtral 24B Q4_K_M comme remplacement de Whisper.
...
Le ``corpus.jsonl`` produit suit la convention v2 du loader
...
Usage::
    ...
    python benchmark/build_corpus_voxtral_val_30.py            # construit
    python benchmark/build_corpus_voxtral_val_30.py --dry-run  # affiche le pick
...
def main() -> int:
    ...
    n_written = _write_corpus(selection)
...
def _write_corpus(selection: dict[str, list[dict]]) -> int:
    """Copie les WAVs sélectionnés vers ``corpora/voxtral-val-30/`` et
    écrit le ``corpus.jsonl`` consolidé. Chaque ligne JSONL réutilise
    l'entrée originale de la télémétrie et ajoute deux champs :
      ...
      - ``payload.tier_validation`` : le bucket de stratification utilisé
        ...
        word-count.
      - ``payload.reference_text_gemini`` : vide, à remplir par la passe
        ground-truth Gemini (sous tâche dédiée).
    """
    ...
    with jsonl_path.open("w", encoding="utf-8") as fout:
        for bucket_name in (b.name for b in BUCKETS):
            for s in selection.get(bucket_name, []):
                ...
                entry["payload"]["reference_text_gemini"] = ""
