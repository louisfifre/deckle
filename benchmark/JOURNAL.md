...
**Cartographie des défauts Voxtral T1 sur le corpus.** Un sub-agent Claude qualitatif a comparé les 30 sorties `reference_text_whisper` (Whisper large-v3 sticky avec initial_prompt benchmarké, version de prod) face aux sorties Voxtral T1_baseline du run 0004. Verdict initial : Whisper l'emporte au critère « dépôt-et-oublie » par robustesse aux pires cas, malgré qualité moyenne presque à parité. Deux défauts Voxtral disqualifiants identifiés : **troncature en milieu de phrase sur audios longs** (audios f11dd93e à 39s et 11d25e0e à 71s, verdict judge Gemini explicite : « la transcription est incomplète et s'arrête brutalement avant la fin de l'audio »), et **hallucinations en queue de transcription** (« Next break. » sur 74131937, « PogVoxra » pour POC Voxtral). Whisper, lui, garde son défaut signature : hallucinations YouTube-subtitle sur silences (« Sous-titrage Société Radio-Canada » sur d8035ca0), confirmées expérientiellement par Louis qui les vit en prod.

**Cas archétypal validant la doctrine « manifest vs invisible ».** Sur audio 677a1dee (« la log window »), Whisper transcrit « Halloween » (substitution invisible plausible — pire selon la doctrine), Voxtral « low window » (faute manifeste, repérable d'un coup d'œil). Voxtral perd au WER mais gagne sur le critère de visibilité de l'erreur. Cas inverse : sur silence (1.6s), Whisper produit une phrase entière trompeuse, Voxtral un fragment « Je... » visible et inoffensif. Les trois critères de fidélité actés ce tour ont été inscrits dans le `CONTEXT.md` racine du projet (section « Transcription — fidelity criteria ») : **grammatical number fidelity**, **plausible reformulation**, **manifest vs invisible error**.

**Identification du bug de bench responsable de la troncature.** Le paramètre `max_new_tokens_per_audio_s = 4.0` dans `lib/sources/voxtral_transformers.py` dimensionnait le budget de génération à ~4 tokens par seconde d'audio (plancher 128). Pour l'audio f11dd93e à 39.3s, ça donnait `max(128, 157) = 158 tokens`. Or la référence Gemini fait 649 caractères ≈ 200-260 tokens. La reco communauté HuggingFace observée dans la recherche externe vise ~6.8 tokens/s (max_new_tokens = 2048 pour ~5 min d'audio). **Hypothèse : la troncature observée vient du bench, pas du modèle.**

**Sandbox de validation isolé** dans [`benches/voxtral-transformers/sandbox_sampling.py`](benches/voxtral-transformers/sandbox_sampling.py). Trois configs testées sur trois audios diagnostiques (greedy max4×, greedy max8×, sampling temp=0.2 max8×). Résultat sur f11dd93e : la baseline coupait au mot « Rel » (HIT_CAP au token près), max8× termine proprement « Relance la phase 2 pour voir. On va voir. ». Sur 74131937 et a66c3e92, sorties identiques entre les trois configs au caractère près — l'hallucination « Next break. » et « Je... » sont stables, indépendantes de max_new_tokens et du sampling à température basse.

**Run 0005-nojudge confirme à grande échelle.** Fix appliqué (`4.0` → `8.0`), bench T1_baseline relancé sans Gemini judge sur les 30 audios. 0 fail, 152s. Comparaison audio par audio vs run 0004 :

| audio | dur | wer 0004 | wer 0005 | hit_cap 0004 | gain |
|---|---|---|---|---|---|
| f11dd93e | 39s | 0.40 | 0.30 | HIT | -10 pts |
| 11d25e0e | 71s | 0.30 | 0.23 | HIT | -7 pts |
| 5e75a75b | 50s | 0.22 | 0.22 | HIT | 0 (cap touché en queue, EOS naturel) |
| 74131937 | 65s | 0.29 | 0.29 | HIT | 0 (idem) |
| 26 autres | — | — | — | — | sortie identique au caractère près |

WER moyen global : 0.308 → 0.302. Gain ciblé sur les troncatures, aucune régression visible ailleurs — observation **cohérente avec** le greedy déterministe, sans en constituer une preuve générale (un seul échantillon de configuration testé en sandbox sampling, pas d'exploration température × top_p × seeds). Bench `voxtral-transformers-validation-0006` avec Gemini judge sur T1+T3 lancé en fin de session pour confirmer apple-to-apple, résultat à analyser à la reprise.

**Verdict révisé.** La troncature, qui pesait lourd dans le verdict du sub-agent qualitatif, était un bug du bench. Une fois corrigé, Voxtral repose sur des défauts moins disqualifiants : hallucinations fragmentaires visibles (« Je... », « Next break. ») plus toxiques en quantité que les hallucinations YouTube-subtitle longues et bien formées de Whisper (« Sous-titrage Société Radio-Canada ») selon le critère manifest-vs-invisible. **Le verdict Whisper > Voxtral pour Deckle T1 est partiellement renversé**, à valider sur le run 0006 avec judge. Hypothèse à confirmer : la doctrine de Louis sur l'erreur visible préférée à l'erreur invisible pourrait faire pencher la balance vers Voxtral une fois la troncature retirée.

**Recherche externe parallèle sur les benchmarks publics.** Sub-agent dédié confirme que Voxtral Mini 3B bat Whisper large-v3 sur les benchmarks ASR FR publics (FLEURS 4.87 vs 5.55, CommonVoice 8.92 vs 11.33 selon paper Voxtral arXiv 2507.13264, écart confirmé et creusé par l'ASR Leaderboard indépendant arXiv 2510.06961 v4). Mais ces benchmarks sont sur de la lecture studio ou du parlé semi-structuré ; le corpus Deckle est du spontané. Aucun chiffre public ne tranche sur le spontané. **L'impression initiale de Louis « Voxtral pas aussi bon que Whisper » n'est pas confirmée par les benchmarks** — soit elle vient des défauts qualitatifs spécifiques (substitution invisible « Halloween », réformulation plausible « en même temps »), soit d'un effet TTS-overfitting réel sur le spontané non capturé par les benchmarks publics. Le bench Deckle est précisément l'instrument adapté pour trancher, et c'est ce qu'on construit.

**Pivot stratégique ouvert vers ONNX/Phi-4.** Direction long terme exprimée par Louis : Native Windows via ONNX Runtime + DirectML comme épine dorsale Deckle (ASR, TTS, correction orthographique, LLM de réécriture, VAD). Avantages structurels : binding C# natif (pas de sous-process Python), GPU AMD via DirectML sans ROCm tatillon, écosystème Microsoft cohérent. **Voxtral n'a pas d'export ONNX officiel à ce jour** — devient un cul-de-sac de production indépendamment de son verdict qualité. Phi-4-Multimodal-Instruct identifié comme candidat ASR sérieux (qualité FR comparable à Voxtral Mini selon les benchmarks publics, ONNX officiel Microsoft, ~13 GiB BF16). Sub-agent recherche état-de-l'art Phi-4 lancé en fin de session — verdict d'intégration .NET production à analyser à la reprise. **Pas de décision tranchée à ce stade** ; reste hypothèse de direction à valider sur chiffres concrets.

**Honnêteté épistémique notée comme correction de posture.** Pendant la session, sur-affirmation diagnostiquée et corrigée par Louis : assertions du type « greedy déterministe confirmé » formulées sur une base d'observation ponctuelle (sandbox 1 seed × 1 température × 3 audios). Reformulation acceptée : « cohérent avec X » plutôt que « X confirmé ». Garde-fou à appliquer systématiquement sur les conclusions tirées de benchs en cours.
...
## 2026-05-27 (suite 4) — Pivot voxtral-transformers vers chat-mode, paliers de validation, baseline T1-T5 relancée

Le run `voxtral-transformers-validation-0001` (T1-T6 × 30 samples judge corrigé) a été lancé sans vérifier que la source `voxtral-transformers` supportait les régimes T2-T6. La docstring de `lib/sources/voxtral_transformers.py` documentait explicitement que seul T1 était implémenté ; cette information a été masquée par l'enthousiasme du « lancer la grande passe » du handoff précédent. Conséquence : T2-T5 ont retourné la même sortie que T1 (le mode `apply_transcription_request` ignore le user_prompt), le judge a évalué ces sorties identiques selon les rubriques de chaque régime, T6 a échoué proprement avec un message clair. Coût Gemini ~$0.05-0.30 estimé pour zéro information utile au-delà de T1. Apprentissage méthodologique livré : avant tout gros run, **protocole de validation incrémentale en paliers** — palier 1 (extension de code), palier 2 (smoke test 1-3 samples sans judge, lecture qualitative), palier 3 (smoke test petit échantillon avec judge), palier 4 (grande passe). Le bench `voxtral-validation` accueillait déjà `--limit` et `--skip-judge`, l'infra était là, c'est la discipline qui manquait.

**Bug d'écrasement silencieux des runs corrigé** dans `lib/paths.py`. Le regex de `next_run_id` exigeait `[a-z0-9]+` sans tiret pour le `model`, donc tout run dont le nom de source contient un tiret (`voxtral-transformers`, `voxtral-llamacpp-mini3b-q8`) n'était jamais reconnu — retour systématique à 1, combiné à `mkdir(exist_ok=True)` + `open("results.jsonl", "w")` en mode write, tronquait silencieusement les runs existants. Diagnostiqué quand le handoff anticipait `0002` mais le bench créait `0001` en écrasant. Parse via préfixe `{model}-{phase}-` et vérification `\d{4}$` désormais. Le run BF16 T1-only antérieur (mentionné dans le handoff précédent comme « précieux runtime ») a été perdu dans l'écrasement, mais le nouveau run le subsume — pas de perte de signal utile.

**Extension de `voxtral-transformers` au chat-mode**. Routage par heuristique : prompt et system_prompt tous deux vides → `apply_transcription_request` (canonique). Sinon → `apply_chat_template` (chat / instruct). T1_baseline reste canonique (prompt vidé dans le TOML pour le forcer en canonique sans casser la sémantique des régimes), nouveau `T1bis_chat` ajouté avec l'ancien prompt T1 pour mesurer canonique vs chat à instruction constante. T6 abandonné : la concaténation `system\n\nuser` dans un seul bloc user (seule voie offerte puisque Voxtral ne supporte pas le rôle system séparément, cf. model card Mistral) ne reproduit pas un vrai system prompt — sortie observée `[?]` sur sample propre au lieu de transcription + étiquette ton. Régime rayé du TOML.

**Filter `very-short × chat-mode` dans le bench**. Voxtral en chat-mode dégénère sur les samples silencieux : réponses templates identiques entre samples (« La personne mentionne que les gens ont tendance à se sentir plus à l'aise avec les robots… » sortie verbatim sur deux audios « Hmm. » différents en T4_summary), dérive 128 tokens sur T6, traduction conversationnelle « I'm sorry, but I can't help you with that. » sur T3. Le mode canonique reste robuste sur ces samples (au pire WER 1.0, jamais de dégénération longue). Décision : régimes chat skippés sur tier `very-short`, T1 canonique traverse tous les tiers.

**Palier 2 (smoke test 3 samples sans judge)** : 21 invocations, 0 fail, 30s. Routage canonique/chat fonctionnel sur les 7 régimes. Sur `dcad692a` (1.7s « Et toujours douter un peu. ») : T1 textuellement identique à T1bis, T3 = traduction anglaise propre, T4 = résumé en une phrase, T5 = « Hésitant ». T6 = `[?]` (confirmation de l'abandon).

**Palier 3 (smoke test 13 samples avec judge)** : 23 invocations (11 T1-only sur very-short + 2×6 sur short), 0 fail, 104s. Judge se branche bien sur les nouvelles sorties chat. Sur `9bbfc858` (11.3s, contenu technique « j'ai largement les 7 gigas »), T1 et T1bis produisent **exactement le même texte mot pour mot**, judge axes identiques 100/100/100/100 — confirmation directe que le chat-mode à prompt constant ne dégrade pas sur signal propre. Observation tech intéressante sur `7c2a983c` : T1 canonique préserve l'anglicisme « settings », T1bis lisse en « réglage ». Judge mesure : T1 fid_signal=90, T1bis 85. **Premier signal mesuré que le mode canonique préserve mieux le vocabulaire original**, important pour la dictée tech de Deckle.

**Palier 4 (grande passe T1-T5 × 30 samples avec judge corrigé)** : `voxtral-transformers-validation-0004`, **125 rows / 0 fail / 861.9s** (14:21 min). Composition : 30 T1 canonique sur tous les tiers + 95 invocations chat (T1bis + T2 + T3 + T4 + T5 sur les 19 samples non-`very-short`). Run archivé sous `%LOCALAPPDATA%\Deckle\benchmark\runs\voxtral-transformers-validation-0004\`. Contenu non analysé en détail dans cette session — c'est le matériau de la prochaine.

**Observations méthodologiques notées :**

- **Variance Gemini significative** : jusqu'à ±100 pts sur samples silencieux entre runs sur le même contenu textuel. Sur `a66c3e92` (« Hmm. »), T1 sortant « Je... » a été noté 100/100/100/100 sur run 0001, 0/50/0/0 sur run 0003. À intégrer dans la lecture : les scores judge sur `very-short` sont volatiles ; au-delà du tier `short`, la variance est plus contenue.
- **Tier classification du corpus ne suit pas strictement la durée**. `bc08abb2` (12.9s) classé `very-short`, `9bbfc858` (11.3s) classé `short`. Probablement basé sur le nombre de mots de référence. À ne pas confondre avec la durée brute lors des analyses.
- **Sample `d8035ca0` (2.3s) a une `reference_text_gemini` polluée** : la chaîne « Transcris cet audio. » (notre prompt). Faux positif de pré-génération ground truth, à investiguer côté `pregenerate_groundtruth_gemini.py`. Non bloquant pour la suite.

**Outillage de lecture des verdicts mis en place** : viewer HTML `<details open>` par défaut, script `aggregate_verdicts.py` (triage par axe judge bas + comptage mots-clés diagnostiques par régime). Test sur run 0003 fait émerger les signaux attendus — `paraphrase` pour T1bis/T2, `anglicisme` pour T3 sur traduction technique, `audio-silence` pour T1 sur `very-short`. Brique de base pour la boucle d'optimisation des prompts (autoresearch Karpathy adaptée) discutée en fin de session.

**Renommages techniques** : worktree `voxtral-llamacpp-poc` → `voxtral-bf16-poc`, branche `poc/voxtral-llamacpp` → `poc/voxtral-bf16`, TOML `voxtral_llamacpp.toml` → `voxtral_validation.toml`. Le verdict 2026-05-27 (suite 3) actait « Q8 abandonné, BF16 confirmé » — le nom de l'arbre de travail le matérialise désormais.

**Direction prochaine session** : lancer `aggregate_verdicts.py voxtral-transformers-validation-0004` et générer le viewer HTML → identifier les 3-5 patterns d'échec dominants → proposer 2-3 variantes du prompt T1_baseline qui adressent ces patterns → relancer, comparer, garder la meilleure. Première itération de la boucle Karpathy adaptée. La métrique de fitness reste à calibrer (proposition à discuter : composite pondéré sur les 4 axes judge). Mise en corrélation avec Whisper large-v3 disponible sans nouveau bench (les `reference_text_whisper` sont déjà dans le corpus). Contrainte doctrine à honorer : **Gemini reste outil upstream (pré-génération, mesure, identification de patterns), jamais runtime utilisateur** — axe d'émancipation des services payants du projet Deckle. Les enseignements informent les prompts statiques Ollama (réécriture), ils ne s'invoquent pas en runtime.

Commits de la session sur `poc/voxtral-bf16` : `ce55fcf` (fix paths), `332eb7f` (source chat-mode + régimes), `32e1050` (outillage verdicts), `1bf3ab3` (scripts ad-hoc).
