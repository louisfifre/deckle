...
type: study
module: benchmark/studies/perf-cap
---
...
These scripts predate the code/data split and write to worktree-local `..\models-cache\` and `..\runs\perf-cap\` — paths now relative to `studies/perf-cap/`, not the benchmark root, and therefore **stale**. The folder is a frozen capture; if the characterization ever resumes, port it onto [`../../lib/paths.py`](../../lib/paths.py) first.
