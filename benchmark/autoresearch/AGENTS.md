...
This folder is for generic autoresearch loops, not for a model-family study. Follow `D:\skills\global\autoresearch\SKILL.md` when creating or running a campaign:
...
If the artifact being evaluated is ASR-specific, keep its corpus, prompts, sources, judges, and metrics under `benchmark/asr/`. Use this folder only when the reusable object is the autoresearch loop itself.
