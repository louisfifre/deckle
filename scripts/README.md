...
| [`lib/inspect-context.ps1`](lib/inspect-context.ps1) | Inventory every tracked Markdown document by role, exact size and line count, plus a model-independent token estimate. Prints totals by document type and a per-file breakdown. | `-Target <worktree>`, `-Pick`, `-Json <path>` |
