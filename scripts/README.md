...
| [`lib/inspect-context.ps1`](lib/inspect-context.ps1) | Inventory every tracked Markdown document by loading mode, additions over 1/7/30 days, heading count, exact size and line count, plus a model-independent token estimate. Separates automatically scoped instructions from references that must be opened explicitly. | `-Target <worktree>`, `-Pick`, `-Json <path>` |
