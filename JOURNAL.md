...
Sketched the target for when sharing lands (refined by Louis, same day): the manifest keeps structure and canonical English keys AND English labels — provisioning always creates canonically — and translation is a single relabel pass driven by per-language token→label tables (`names.fr.json`, …). That unification is the point: provisioning in French and switching a live space to Dutch are the same operation (a possible `schema_relabel` verb), one code path, never a migration — made safe because keys are immutable while labels rename freely. Language is chosen at Deckle first-run setup, pre-selected from the OS display language, overridable (Louis: English OS, French spaces); contributing a language means adding one file, machine-translation acceptable as a first draft. Extraction from today's inline form stays mechanical — the keys already are the tokens.
