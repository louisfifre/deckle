...
**Matériau de recherche brut** — deux zones distinctes selon l'usage. `docs/research/research--slug--YYYY-MM-DD.md` reçoit du matériau de recherche brut, peu importe la source — typiquement une recherche profonde lancée en parallèle sur un LLM tiers (ChatGPT, autre) pour économiser des tokens du modèle principal, ou plus rarement le retour d'un sous-agent interne quand on veut archiver son investigation. On y trouve des données collectées, des analyses préliminaires, des survols de sujet, des comparatifs d'options — **jamais une synthèse close ni une décision tranchée**. Première ligne en blockquote pour situer la provenance (`> Source : ChatGPT GPT-5 · 2026-05-15 · prompt original archivé en fin de fichier`). `docs/references/external/external--sujet--source-YYYY-MM-DD.md` reçoit une référence importée de tiers qu'on garde telle quelle pour la citer plus tard depuis un ADR ou une fiche reference. Ces fichiers ne sont jamais des sources de vérité — ils sont des artefacts qu'on consulte, qu'on cite éventuellement, et qu'on garde tels quels.

Conséquence opérationnelle pour Claude : si une enquête (locale ou via sous-agent) aboutit à une **conclusion** ou à une **décision** — par exemple « le bug X n'existe plus, on lève la contrainte qu'il imposait » — la trace écrite va directement en **ADR**, pas dans une note research intermédiaire. Le matériau brut qui a alimenté l'enquête peut accessoirement vivre sous `docs/research/` s'il a une valeur d'archive future, mais ce n'est pas le mode par défaut.
