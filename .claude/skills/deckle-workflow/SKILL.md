...
A push to GitHub is bound to a **version**, never to "a coherent state landed locally". Outside a version cut, `main` accumulates merges locally and is not synchronized; the decision to cut a version belongs to Louis. **`main` receives only merges of coherent states tested in usage** — a fresh clone of `main` runs at all times; unripe work lives in a local branch or worktree. The version bump and annotated tag `vX.Y.Z` are rare acts, reserved for perceivable milestones (delivered feature, completed structural refactor, stable fix batch tested in usage) — **SemVer 2.0: the version measures compatibility.** `MAJOR.MINOR.PATCH` — MAJOR breaks, MINOR adds without breaking, PATCH fixes without breaking. Deckle has no public API or downstream consumer, so compatibility is read at the **user/behaviour** level, not API. The `0.x` phase lets anything change; Deckle's reading within that latitude (a project convention, not the spec): MAJOR = overhaul (UI, backend, paradigm), MINOR = a significant cycle (a feature, an engine or architecture change), PATCH = a fix or a small step — no `1.0` before an installer and a stable behaviour surface. The workflow: edit `<Version>` in `Deckle.App.csproj` (single source), commit `chore(release): vX.Y.Z`, annotated tag `git tag -a vX.Y.Z -m "Release vX.Y.Z"`, push branch then tag. The native bundle `native-vX.Y.Z` follows its own cycle.

**Changelog — keepachangelog, paired with SemVer.** A version cut adds a curated entry to `CHANGELOG.md` (root), authored with the `chore(release)` commit: one entry per version, newest first, ISO date `YYYY-MM-DD`. The entry is **curated for the reader, never a raw `git log` dump** — the Conventional Commits history is the raw material a generator drafts from (`git-cliff` fits: single Rust binary, keepachangelog output, no CI needed), then the entry is edited by hand. Closed vocabulary, six categories in order: **Added** (new capability), **Changed** (behaviour of an existing one), **Deprecated** (present, slated for removal), **Removed** (gone), **Fixed** (bug), **Security** (vulnerability). Only these six. Any extension — e.g. a `Known issues` block for an experimental opt-in shipped to testers — is non-canonical: place it after the six and mark it explicitly as a Deckle extension; the default is to fold the caveat into the entries instead. Deprecation discipline (SemVer FAQ): announce under `Deprecated` in a MINOR, keep at least one cycle, then `Removed` in a MAJOR — never remove cold. **Release notes** — user-facing, narrative, benefit-framed, living in the GitHub Release body or in-app — are a distinct artifact *derived* from the changelog; their doctrine is deferred.
