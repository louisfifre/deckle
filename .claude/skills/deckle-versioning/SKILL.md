...
Every completed workstream advances the version so the installed build remains traceable. Magnitude follows the user-facing change rather than API compatibility: MAJOR is an overhaul, MINOR a real cycle (a feature or engine change), and PATCH is a fix, internal change, documentation workstream, or other small step. Stay in 0.x until the behaviour surface is stable enough to promise.
