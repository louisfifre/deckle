...
The detailed architecture (shell + 3 pages, Tag → Type.GetType pattern, partial MVVM targeted at what pays off, window lifetime Hide-on-close + dispose page resources on terminal Closed) follows exactly the pattern of `Deckle.Settings` — read `src/Deckle.Settings/CLAUDE.md` before touching the shell or the routing. The Settings doctrine (Auto-save everywhere, NavigationView Auto, SettingsCard / SettingsExpander, H1 header per page) does not apply verbatim here because the Playground is not a final user settings page — it is a tuning workshop with dense sliders, live previews, and programmatically generated panels.
...
**HUD toolbar: grouped DropDownButton + enable-bound transport.** The target picker is a `DropDownButton` with a `MenuFlyout` containing two `MenuFlyoutSubItem` ("States" for Charging/Recording/Transcribing/Rewriting, "Primitives" for Conic/ArcMask/Combined). The button label reflects the current target via `{x:Bind ViewModel.CurrentTargetLabel}`. The transport is three explicit Play / Pause / Stop buttons whose `IsEnabled` is bound to computed properties of the VM — no toggle that swaps its icon under the cursor, the user always sees which actions are possible. The tuning panel only shows the `SettingsExpander` relevant to the active target (mapping defined by `HudViewModel.ActiveTuningSections`), with the Parked expander always appended at the bottom (collapsed by default) for transition values rarely touched.

**Page contract aligned with Settings.** Each page inherits from `Page`, declares `NavigationCacheMode = NavigationCacheMode.Required` in the constructor, and survives navigations in the `Frame`. The runtime resources (naked composition preview, screen capture service, frame sampler, Hue REST output, preview cells) live as instance fields of the page and are disposed via `DisposeResources()` called by `PlaygroundWindow.Closed` (terminal, not Closing→Hide which must preserve state). The `OnNavigatedTo` reloads the ViewModel from the settings service; `OnNavigatingFrom` stops the off-screen UI timers without touching the canonical pipeline that keeps pushing to Hue.
...
Lazy singleton created by `App.OnLaunched` on the first `ShowPlaygroundLazy()`, never destroyed before `QuitApp`. `Closing` cancels + hides via `AppWindow.Hide()` (the Win32 `SW_HIDE` pattern used by Settings is not reproduced here because Playground does not exhibit the same DComp swap chain symptom; to be re-evaluated if the bug surfaces on Playground). `Closed` only arrives at the terminal quit and that is where the pages' `DisposeResources()` are called to release the Win2D composition, the screen capture service, the frame sampler, and unsubscribe the `HuePairingService.BridgeChanged` + `AmbientSettingsService.Changed` observers.
