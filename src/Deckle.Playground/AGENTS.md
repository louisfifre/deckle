...
## Persistence follows the owning surface
...
- **Segmentation tuning is an explicit draft.** `SegmentationViewModel` loads the real transcription settings into an editable snapshot. Changes remain local until Save writes them through `TranscriptionSettingsService`; Revert returns to the last saved snapshot and Reset returns to shipping defaults.
