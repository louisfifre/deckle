...
**Fail open.** Only unmodified physical vertical hook events whose delta is a non-zero multiple of `120`, and whose focused app also owns the pointer target, are consumed. This includes Windows-batched classic detents. Injected events, modified or horizontal input, different targets, Raw Input observations, and finer deltas continue to Windows unchanged. Creation or system-direction failure leaves conversion disarmed. Queue overload detaches the interceptor but finishes the already accepted budget; an injection failure is the exceptional irreversible boundary and stops conversion immediately.
