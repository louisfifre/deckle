...
**`CaptureStallDetector` orphelin.** Écrit comme watchdog acquire-based (`b6abd1d`, 7 tests verts), puis pressenti pour un repurpose en capteur de changement de format. Le scénario A étant confirmé — le funnel recreate détecte le flip inline, sans capteur séparé — ce type n'a plus d'objet. **Retiré** de la branche avant merge (commit de suppression en avant) : il modélisait un stall d'acquire jamais observé — pas le mismatch de format effectivement corrigé, que ce watchdog acquire-based n'aurait d'ailleurs pas vu (les acquires tournent, c'est la sortie sampler qui mourait). Git le conserve sans perte (`git show e419bf7`, tests compris) si le chantier observabilité décide un jour de câbler un watchdog de capture contre le `CaptureLoop` d'alors.
