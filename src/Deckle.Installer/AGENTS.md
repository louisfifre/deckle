...
Its contract is narrow: download the app payload → verify SHA-256 → extract → integrate (Start Menu, Installed-apps, `DECKLE_DATA_ROOT`) → launch. It never touches natives, models, or Ollama — runtime provisioning is the app's own first-run job (`Deckle.Setup`). Two folders, the whole UX point: binaries (per-user) and data/models (relocatable off a saturated C: via `DECKLE_DATA_ROOT`).
