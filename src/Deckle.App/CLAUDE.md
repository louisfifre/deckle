...
The lifetime of WinUI 3 windows in Deckle is driven by `Closing→Cancel`. All Windows (HUD, LogWindow, SettingsWindow, PlaygroundWindow) block their own close. The single exit point is the tray's Quit menu, which calls `QuitApp()` (releases tray, message host, engine, then `Environment.Exit(0)`). Consequence: a `LogWindow` that was never shown has no initialised layout — `LogScrollViewer.UpdateLayout()` can only be called after the window has been shown at least once (flag `_isVisible` in place).
...
`LogWindow.xaml(.cs)` is the live window for visualising EventSource events. Resizable `OverlappedPresenter`, min 400×300, `MicaBackdrop`, system theme (light/dark auto, no forced `RequestedTheme`). Close → Cancel + Hide like all long-lived Windows.

Native TitleBar `Microsoft.UI.Xaml.Controls.TitleBar` (WindowsAppSDK 1.8) with **Tall** caption buttons (`AppWindow.TitleBar.PreferredHeightOption = TitleBarHeightOption.Tall`). The app icon lives in `TitleBar.IconSource`: `ImageIconSource` fully rebuilt on each idle/recording toggle (mutating `ImageSource` in place does not propagate visually). `AppWindow.SetIcon` follows the same state. The live-search `AutoSuggestBox` is placed in `TitleBar.Content` — Win11 Settings pattern. Below 520 DIPs of width, the SearchBox switches to an icon-only button (`TitleBar.Content` swap), Win11 Task Manager pattern.
...
The order in `OnLaunched` is sensitive because it crosses several invariants: settings migration must run before any service touches its file, the `MessageOnlyHost` must exist before hotkeys are registered, the tray must have its callbacks wired before its `Register`. The canonical sequence is: migration `SettingsBootstrap.MigrateLegacyToPerModule()` first, then registration of `TelemetryGates.Configure` and logging sinks, then first-run gate (wizard if natives or models are missing), then instantiation of `TranscriptionEngine`, then creation of long-lived windows (HUD eagerly, LogWindow and SettingsWindow and PlaygroundWindow stay lazy), then creation of the `TrayIconManager` (callbacks only, not yet `Register`), then wiring engine events → tray + windows, then creation of the `MessageOnlyHost`, then `tray.Register(messageHost.Hwnd)` and `hotkeyManager.Register()`, then application of the persisted theme and of the calibration level window, then conditional opening of Settings if `--settings` is passed on the CLI.

Three global diagnostic safety nets are set up in the `App` constructor: `Application.UnhandledException`, `AppDomain.UnhandledException`, `TaskScheduler.UnobservedTaskException`. All three route to `LogService` with `LogSource.Crash` and a distinct prefix (`CRASH`, `CRASH-AD`, `CRASH-TS`). Without these safety nets, an exception that surfaces in a `TranscriptionEngine` handler can disappear silently — the pattern existed before telemetry was unified and remains in place for cases where the main sink is not yet registered (early boot).
