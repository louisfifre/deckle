...
Settings > Diagnostics is the sole editing authority. It presents three distinct groups: **Logging details**, **Application log**, and **Telemetry**. Logging details initially contains four off-by-default activity controls: Ambient, Transcription, Autocorrect, and Windowing. Module pages and the LogWindow never duplicate them. The LogWindow shows a compact read-only indication when details are disabled and routes to Settings.
...
Windowing is one technical diagnostics control with no human timeline and no sub-toggles. Placement, anchoring, z-order, resize, Win32 enumeration, timing, and payload construction are gated at their producers. Trackpad frame-rollup and HUD proximity-rollup loops are the next candidates to audit; provider existence alone never earns a new setting.
