...
- Named icons are refused on OBJECT creation (400 "icon name and color are not supported for object"); only emoji icons pass. Section icons are therefore emoji-only, rejected at manifest parse with corrective guidance — the named-icon grammar belongs to types.
