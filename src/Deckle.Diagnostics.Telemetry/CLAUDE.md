...
- **`CorpusEnabled`** — bool, off par défaut. Gate l'écriture du texte brut de transcription dans `<UserDataRoot>/corpus/<profile>/corpus.jsonl`.
- **`RecordAudioCorpus`** — bool, off par défaut. Gate l'écriture du WAV brut à côté de la ligne corpus correspondante. Coût disque non négligeable, consentement à demander.
