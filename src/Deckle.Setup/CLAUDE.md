...
Wizard first-run de Deckle. `SetupWindow` (shell trois-rows : header + Frame + footer Cancel/Back/Next) avec trois pages frame-navigées (`ChoicesPage`, `InstallingPage`, `SummaryPage`). Le module porte son propre `SetupContext` (état de wizard partagé entre les pages) et orchestre des primitives de provisioning fournies par les autres modules : `Downloader` et `ModelEntry` (générique) côté `Deckle.Transcription`, `NativeRuntime` et `SpeechModels` (whisper-spécifique) côté `Deckle.Transcription.Whisper`. Détaché de `Deckle.App` en cartographie-cleanup pour que la `GeneralPage` de Settings puisse rouvrir le wizard sans que l'app hôte traîne le XAML.
...
Découplage volontaire : le wizard UI vit dans ce module, mais les primitives qui *exécutent* le provisioning whisper-spécifique (`NativeRuntime`, `SpeechModels`) vivent dans `Deckle.Transcription.Whisper/Setup/` aux côtés de l'implémentation `IAsrBackend`. Le wizard les référence directement. Quand un second backend ASR ship (Voxtral), il portera ses propres primitives de provisioning dans son module enfant, et le wizard sélectionnera l'ensemble approprié selon le backend choisi.
