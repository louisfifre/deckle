...
**Central guardrail** (the reason the defaults are gentle): compressing hard would raise the inter-word noise floor, and a lifted noise floor is documented as fuel for Whisper's silence hallucinations — the spurious « Sous-titres réalisés par… » boilerplate (cf. [`docs/research/research--whisper-dynamic-vad-distil-fr--2026-05-28.md`](../../docs/research/research--whisper-dynamic-vad-distil-fr--2026-05-28.md)). Hence the ~2:1 ratio, the gate off by default, and a conservative target. All stage parameters would be conservative starting points to be refined by the WER bench, not values asserted as optimal.

The sub-module is **pure — it emits nothing**. It has no `EventSource` dependency, so the module invariant "all emissions go through `DeckleAudioSource.Log`" stays intact: the observability of a processed take is emitted by the *orchestrator* on its own provider (`DeckleWhispSource.TranscriptionPreprocessed`, Verbose / `Pipeline` keyword), built from the `PreprocessingResult` the function hands back. Activation transitions are emitted the same way (`DeckleWhispSource.PreprocessingActivationChanged`, Informational / `Capture` keyword).

**Settings.** `CaptureSettings.Preprocessing` (`PreprocessingSettings`, written with auto-properties so it round-trips cleanly through `JsonSettingsStore` — unlike the field-based `LevelWindowSettings`) carries the black-box `Enabled` toggle (**false** by default), the per-stage parameters tuned at the Playground, and the activation state. It persists with the rest of the module under `modules/audio/settings.json`.

**Deferred activation** — *enabling is not the same as being active*. `PreprocessingActivation { Calibrating, Active, Dormant }` plus the pure `PreprocessingActivationCalculator` (mirrors `MicrophoneCalibrationCalculator`). On opt-in the state is `Calibrating` and the DSP does **not** transform; the orchestrator accumulates mic telemetry over the first few takes (`TranscriptionEngine.PreprocCalibrationTakes`, provisionally 5, in a ring next to the auto-calibration buffer). The calculator takes the median of the per-take `MeanDbfs`, computes the delta to `TargetRmsDbfs`, and if the mic sits at least `activationDeltaDb` below target (`DefaultActivationDeltaDb`, provisionally 6 dB) it would switch to `Active`; otherwise `Dormant` (« your mic is already at a good level »). The threshold is a **signal-based** criterion — defensible on its own, independent of any WER figure — but its exact value stays provisional until the bench measures it. The Recording page states the deferred model plainly so the toggle does not read as broken during calibration.

**Corpus integrity** ([ADR-0011](../../docs/adr/0011-corpus-normalise-comme-dataset-ml.md)). When the stage is active the orchestrator processes a *separate* `backendAudio` buffer and feeds that to the backend; the raw `audio` buffer stays the one written to the corpus WAV. The corpus therefore keeps an untouched raw baseline — exactly what the WER A/B bench needs to compare raw vs DSP on the same takes. The warmup path (embedded TTS clip) is never pre-processed: it is already clean.
