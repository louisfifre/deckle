...
**Self vs external discrimination.** The bridge re-emits our own `PUT` calls to the SSE flow — without discrimination we would have a false positive that stops the engine on every one of our own pushes. The chosen pattern is *state echo classification*: `AmbientEngine` stores the Hue state it has just pushed, namespaced by `group:<v1_id>` or `light:<v1_id>`, with the local `DateTimeOffset.UtcNow` push timestamp. On event receipt, `AmbientHueEchoClassifier` compares the partial EventStream payload (`on`, `bri`, `xy`) with that last pushed state. **The state match is authoritative, the age is not a gate.** If the payload matches within tolerance it is our own echo, ignored whatever its age; if it differs on any carried component it is an external change and the engine stops. The last push is our standing intent for a light until the next push, so a stale slot still reflects what ambient wants on screen and the age carries no signal — it survives only as the telemetry `age_ms` on `ExternalChangeStoppedDetail`/`EchoIgnored`. The earlier design ANDed the match with a fixed 2 s window; on a static zone the per-light delta-gate suspends pushes, the stored slot goes stale, and a delayed bridge echo past that window was wrongly read as external, stopping the engine itself (incident 2026-06-04, pinned by `AmbientHueEchoClassifierTests`). The Hue `creationtime` stays informational only — local push-vs-receipt timing avoids bridge/host clock skew.
