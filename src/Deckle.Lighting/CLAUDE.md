...
**Self vs external discrimination.** The bridge re-emits our own `PUT` calls to the SSE flow — without discrimination we would have a false positive that stops the engine on every one of our own pushes. The chosen pattern is *local timestamp*: `AmbientEngine` tracks `DateTimeOffset.UtcNow` at the moment each push succeeds, namespaced by `group:<v1_id>` or `light:<v1_id>`. On event receipt, the engine compares `UtcNow` to that last push timestamp for the resource concerned — if the gap is < 300 ms (`EchoWindow`), it is our own echo, ignored. Beyond that, it is an external change and we stop. Comparing to `UtcNow` rather than `event.creationtime` avoids all bridge/host clock skew problems: both timestamps are in the same clock.

**v2 ↔ v1 map.** EventStream events carry v2 ids (UUIDs), whereas REST push uses v1 ids (integers). `HueBridgeClient.FetchV2IdMapsAsync` does a single fetch at engine startup and returns two dicts (`Lights` v2_uuid → v1_id, `GroupedLights` v2_uuid → v1_group_id) that the engine caches for the whole session. If a lamp is added to the bridge mid-session, we miss its event — acceptable, the mapping refreshes on the next `StartAsync`. If the fetch fails at init (rare, old firmware or weird network), the engine logs `ReclaimSetupFailed` Warning and continues without external detection — normal pushes still work, just without automatic stop on external command until the next session.
