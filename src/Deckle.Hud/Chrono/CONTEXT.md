...
Shared vocabulary and the authoritative colour-state model for the chrono face (`HudChrono`). The face is the `MM.SS.cc` clock inside the HUD card. This file fixes the terms Louis and the agents use, and the single matrix that decides what colour every element takes in every phase. When the colour logic in `HudChrono.*.cs` and this matrix disagree, this matrix wins.
...
Each digit has an **accent twin** overlaid in the same cell (`Min1Accent`, `Sec2Accent`, …), carrying the exact Windows accent colour at `Opacity=0` until a reveal lifts it. The swipe animator indexes the six digits `0→5`: Min1, Min2, Sec1, Sec2, Cs1, Cs2.
