...
Each chrono digit that changes at least once switches to red `SystemFillColorCriticalBrush` (Windows theme resource, follows light/dark) and stays there until the next clock reset (`ResetClock` / `StartClock`, see *Clock lifecycle* below). The initial `0` stays neutral as long as it has not moved. Idle Runs do not carry a local Foreground — they inherit from `ClockText.Foreground = {ThemeResource TextFillColorPrimaryBrush}`, auto-reactive to the theme. The reset is `ClearValue(TextElement.ForegroundProperty)` on each named Run.

`ChronoRoot.ActualThemeChanged` re-resolves the critical brush and re-applies it to digits already lit (a Foreground assigned in code does not follow a `ThemeResource` binding, it has to be reassigned manually). Cadence: `CompositionTarget.Rendering` (vsync, no `DispatcherTimer` → no jitter). Hooked in Recording to drive the ticking clock, and kept hooked in Transcribing/Rewriting to drive the swipe reveal — there the clock is frozen (`StopClock`), so the per-vsync `UpdateClock` is a no-op on the digit values while `UpdateSwipe` animates. Unhooked only in Charging/Hidden.

## Clock lifecycle (HudChrono)

The chronometer's functional lifecycle — `ResetClock` (zero the face), `StartClock` (restart + tick), `StopClock` (freeze on the final value) — is **separate from the `ApplyState` paint methods**. `ApplyState` only styles the card (Foreground, Composition stroke, swipe); it never starts, stops, or zeroes the clock. The host owns the pairing: `HudWindow.SetState` maps each transition to its clock op (`Charging → ResetClock`, `Recording → StartClock`, `Transcribing → StopClock`) right after the overlay gate, before dispatching the paint; the Playground (`HudPage.ApplyTarget`) drives the same public methods for its previews. This separation is deliberate: it keeps the elapsed value independent of the painted visual, so the same look can be rendered over either a zeroed clock (session start) or a frozen one (end of take) — and it fixes the old bug where a take opened on the previous take's frozen value, which a `Reset()` buried inside `ApplyCharging` used to paper over. `ChronoTimer` (in `Deckle.Chrono`) is the underlying stopwatch and is already complete (`Start` = `Restart`, `Stop`, `Resume`, `Reset`); the lifecycle lives in `HudChrono` because it also owns the digit-face display (glyphs, the changed/heat reset), which the bare timer does not.
...
Show goes through `MoveAndResize` (recomputes DPI on each show) then `ShowWindow(SW_SHOWNOACTIVATE)` + `SetWindowPos(HWND_TOPMOST, SWP_NOACTIVATE|SWP_NOMOVE|SWP_NOSIZE)`. `HWND_TOPMOST` is deliberate: it reasserts the native topmost band on every visible transition while `SWP_NOACTIVATE` preserves the "never steal focus" invariant. **Never `SetForegroundWindow`** for the HUD itself. Hide is `ShowWindow(SW_HIDE)`. The window is created once in `OnLaunched`, never destroyed (`Closing → Cancel`). UI handlers are marshaled through `DispatcherQueue.TryEnqueue` because `TranscriptionEngine` events come from background threads. The overlay is toggleable in Settings (`Overlay.Enabled`), checked at the top of `SetState`.
...
## No HUD composition warm at boot

There is no `PrimeAndHide()` boot path anymore. The former invisible `Charging → Hidden` warm pass paid the first DComp / Bitcount / DWM visual-tree cost at startup, but it also introduced a second synthetic HUD show in the post-build launch path and made z-order diagnosis ambiguous. The model warmup stays on demand in `Deckle.Transcription`; the HUD now only shows when a real app state needs it.

The first real visible transition may pay the composition cost. That path is intentional: `ShowNoActivate()` reasserts topmost and no-activate semantics, then `ApplyShowAlpha` handles the 150 ms fade-in and proximity activation exactly as every later HUD show does. Avoid reintroducing a hidden/off-screen priming cycle unless there is a measured first-frame regression and the post-build topmost path is covered by a bounded repro test.

## Real show — 150 ms fade-in

Any `Hidden → visible` (Charging, Recording, Transcribing, Rewriting, Message) triggers a 150 ms cubic ease-out fade-in (`1 - (1-t)^3`), aligned with `WindowSlideAnimator` / `LayeredAlphaAnimator` of the overlay subsystem. The transition is centralized in `ApplyShowAlpha`. If `!wasShown && AnimationSystemSetting.AreClientAreaAnimationsEnabled()`, `StartFadeIn(MAX_ALPHA)`; during the fade `_proximityActive = false`, at the end proximity is re-activated + immediate call to `UpdateProximity`. Otherwise (state switch in already-visible, or animations off), `SetAlphaImmediate(MAX_ALPHA)` instant + immediate proximity.
...
### Internal instrumentation — four axes

The transverse instrumentation wave (May 2026) wires four Verbose axes on the HUD mechanics, addressing the *HUD internals under-instrumented* gap identified in `reference--eventsource-convention--1.2.md` §*Identified gaps*. Existing Info milestones (none on this provider today) stay intact; Verbose comes in as a technical mirror.

`StateChanged(from, to, reason, alpha, dpi)` — keyword `Lifecycle`. Emitted by `HudWindow.SetState` on every transition between the six states (`Hidden`, `Charging`, `Recording`, `Transcribing`, `Rewriting`, `Message`). `reason` is a semantic tag propagated by every entry point (`show_recording`, `hide_sync`, `message_timeout`, etc.) that lets a LogWindow trace be read back without guessing why the HUD just switched. `alpha` captures the pre-transition `WS_EX_LAYERED` alpha, `dpi` reflects `GetDpiForWindow` at transition time.
