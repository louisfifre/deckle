...
Robustness: the prime MUST run synchronously on the worker thread, never on a detached background thread. The old boot warmup ran on its own thread and raced a real hotkey transcription, which occasionally leaked priming text to the clipboard. Synchronous-on-worker removes the race structurally. The prime now bypasses the pipeline entirely — it calls the backend directly rather than going through a production strategy or the shared `FinalizeTranscription` — so there is no user-facing tail (clipboard, LLM rewrite, paste, corpus, `StatusChanged`, `TranscriptionFinished`) to suppress and nothing to gate. (The earlier `t_isWarmup` ThreadStatic flag that gated that tail is gone; the direct-backend prime makes it unnecessary.)
